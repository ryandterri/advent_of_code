...
const card_values = ['A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J']
...
for (const line of lines) {
  ...
  for (const card of card_values.filter(x => x !== 'J')) {
  ...
  hand.card_value_array.sort((a, b) => a.count > b.count ? -1 : 1)

  const j_count = hand_array.filter(x => x === 'J').length
  if (j_count) {
    if (hand.card_value_array.length) {
      hand.card_value_array[0].count += j_count
    }
    else {
      hand.card_value_array.push({ card: 'A', count: j_count })
    }
  }
